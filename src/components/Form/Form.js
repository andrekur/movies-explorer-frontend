...
function Form({title, onSubmit, children, submitButtonText, additionalText, additionalLinkText, additionlaLink, isValid}) {
  return (
    <div className="form">
      <Logo/>
      ...
      <form className="form__content" onSubmit={onSubmit}>
        { children }
        <button className={`form__save ${!isValid ? 'form__save_disable' : ''}`} type="submit">{submitButtonText}</button>
      </form>
